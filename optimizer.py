...
"how many bikes should move from each surplus station to each deficit
...
def compute_surplus_deficit(stations_df, capacity_col="capacity", current_col="current_bikes_total",
    ...
    Adds predicted_bikes / surplus / deficit columns (surplus = docks that
    will be short, deficit = bikes that will be short, both >= 0 and
    mutually exclusive as long as capacity > min_buffer, true for every
    real station).
    """
    df = stations_df.copy()
...
def solve_rebalancing(stations_df, top_k_neighbors=8):
    ...
    lat, lng, surplus, deficit. Returns a DataFrame of recommended moves:
    from_station_id, to_station_id, bikes, distance_km, minutes -- sorted
    by bikes descending (biggest moves first).
    ...
    """
    sources = stations_df[stations_df["surplus"] > 0].reset_index(drop=True)
    ...
    for _, s in sources.iterrows():
        G.add_node(("src", s.station_id), demand=-int(s.surplus))
    for _, d in sinks.iterrows():
        G.add_node(("snk", d.station_id), demand=int(d.deficit))

    total_surplus = int(sources["surplus"].sum())
    total_deficit = int(sinks["deficit"].sum())
    imbalance = total_surplus - total_deficit  # >0: excess supply, <0: excess demand
    if imbalance != 0:
        G.add_node("dummy", demand=imbalance)
        if imbalance > 0:
            for _, s in sources.iterrows():
                G.add_edge(("src", s.station_id), "dummy", capacity=int(s.surplus), weight=0)
        else:
            for _, d in sinks.iterrows():
    ...
    for _, s in sources.iterrows():
        ...
        for dest_id, km in dists[:top_k_neighbors]:
            ...
            cap = min(int(s.surplus), int(d.deficit))
            if cap <= 0:
                continue
